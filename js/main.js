...
return declare( JBrowsePlugin,
{
    ...
    addTracks: function( tracks, inStartup ) {
        ...
        array.forEach( tracks, function( track ) {
            ...
            function _findCategory( obj, names, path ) {
                ...
                var cat = obj.categories[categoryName] || ( obj.categories[categoryName] = function() {
                    ...
                    obj.pane.own( listener = on( checkDom, 'change', function(){
                        var categoryStr = this.value, categoryList=thisB.categories, isChecked = this.checked;
                            ...
                            for(var i=0; i<catSize; i++){
                                ...
                                if(testCat.hasOwnProperty('box')){
                                    ...
                                    if(catMatch){
                                        ...
                                        //count subcategories
                                        catSize=0;
                                        for(key in testCat.categories){ if (testCat.categories.hasOwnProperty(key)) catSize++;}
                                        if(catSize > 0)
                        ...
                        _updateCategoryTracks(this.value,this.checked);
                        function _updateCategoryTracks(categoryStr, isChecked){ 
                    ...
                    return { parent: obj, pane: c, categories: {}, tracks: {}, box:checkDom };
                }.call(thisB));
            ...
            var checkbox = domConstruct.create('input', { type: 'checkbox', className: 'check' }, labelNode );
            ...
            this.own( checkListener = on( checkbox, 'click', function() {
                ...
                var catList = trackConf.category.split("/");
                for (var j = 0; j < catList.length; j++)
                    catList[j] = catList[j].replace(/^[ ]+|[ ]+$/g,''); //trim leading/trailing spaces
                var cat = thisB;
                ...
                function _countCheckedTracks(cat, catList){
                    //console.log('counting '+catList.length);
                    if(catList.length==0)
                        return [0,0];
            ...
            domConstruct.create('span', { className: 'key', innerHTML: trackConf.key || trackConf.label }, labelNode );
